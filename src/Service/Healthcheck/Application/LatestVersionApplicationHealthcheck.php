...
class LatestVersionApplicationHealthcheck implements HealthcheckServiceInterface, HealthcheckCliInterface
{
    ...
    /**
     * Current master version according to the official cipherguard repository.
     *
     * @var string
     */
    ...
    private function getLatestTagName(): string
    {
        ...
        if (is_null($remoteTagName)) {
            $results = $this->client->get('https://api.github.com/repos/cipherguard/cipherguard_api/releases/latest');
    ...
    {
        $msg = __('Using latest cipherguard version ({0}).', Configure::read('cipherguard.version'));
        if ($this->exceptionThrown) {
            $msg = __('Could connect to cipherguard repository to check versions.');
        }
    ...
    public function getFailureMessage(): string
    {
        ...
        if ($this->exceptionThrown) {
            $msg = __('Could not connect to cipherguard repository to check versions.');
            $msg .= ' ';
    ...
    {
        $msg = __('See https://www.cipherguard.com/help/tech/update');
        if ($this->exceptionThrown) {
