...
{
    public const CIPHERGUARD_PLUGINS_HEALTHCHECK_SECURITY_INDEX_ENDPOINT_ENABLED =
        'cipherguard.plugins.healthcheck.security.indexEndpointEnabled';
    ...
    {
        if (!Configure::read(self::CIPHERGUARD_PLUGINS_HEALTHCHECK_SECURITY_INDEX_ENDPOINT_ENABLED)) {
        ...
        foreach ($resultsGroupByDomain as $domainKey => $checkResults) {
            ...
            foreach ($checkResults as $checkResult) {
                ...
                if ($checkResult instanceof LatestVersionApplicationHealthcheck) {
                    ...
                    $result[$domainKey]['configPath'] = CONFIG . 'cipherguard.php';

                    $result[$domainKey]['info']['remoteVersion'] = $checkResult->getRemoteVersion();
                    $result[$domainKey]['info']['currentVersion'] = Configure::read('cipherguard.version');
                    ...
                    $result[$domainKey]['gpgKeyPublic'] = (Configure::read('cipherguard.gpg.serverKey.public') !== null);
                    $result[$domainKey]['gpgKeyPrivate'] = (Configure::read('cipherguard.gpg.serverKey.private') !== null);
                    $result[$domainKey]['gpgKey'] = (Configure::read('cipherguard.gpg.serverKey.fingerprint') !== null);
                    $result[$domainKey]['info']['gpgKeyPrivate'] = Configure::read('cipherguard.gpg.serverKey.private');
