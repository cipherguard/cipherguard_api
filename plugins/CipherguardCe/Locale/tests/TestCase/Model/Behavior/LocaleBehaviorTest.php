...
class LocaleBehaviorTest extends TestCase
{
    ...
    public function testFindContainLocale(): void
    {
        ...
        UserFactory::make(['username' => 'ada@cipherguard.com'])
            ->withLocale('fr-FR')
            ->persist();

        UserFactory::make(['username' => 'betty@cipherguard.com'])
            ->withLocale('en-UK')
            ->persist();

        UserFactory::make(['username' => 'carol@cipherguard.com'])
            ->persist();

        $user = $this->usersTable->find('locale')
            ->where(['username' => 'ada@cipherguard.com'])
            ->contain('Locale')
            ->first();
        $this->assertEquals('fr-FR', $user->get('locale'));

        $user = $this->usersTable->find('locale')
            ->where(['username' => 'betty@cipherguard.com'])
            ->contain('Locale')
            ->first();
        $this->assertEquals('en-UK', $user->get('locale'));

        $user = $this->usersTable->find('locale')
            ->where(['username' => 'carol@cipherguard.com'])
            ->contain('Locale')
            ->first();
        $this->assertEquals('en-UK', $user->get('locale'));
    }
