...
use Cipherguard\SmtpSettings\Form\CustomSslOptionsForm;
...
class SmtpSettingsSslOptionsGetService
{
    ...
    {
        $values = Configure::read('cipherguard.plugins.smtpSettings.security', []);

        $form = new CustomSslOptionsForm();
        $valid = $form->validate($values);
        if (!$valid) {
            $errors = Hash::flatten($form->getErrors());
            $errorMessage = __('Invalid `cipherguard.plugins.smtpSettings.security` configuration values.');
