...
class SystemCheckControllerTest extends WebInstallerIntegrationTestCase
{
    ...
    public function testWebInstallerSystemCheck_PHP_Below_Next_Version_Warning()
    {
        ...
        $this->assertCanStartSetup();
        $msg = 'PHP version less than ' . Configure::read(NextMinPhpVersionHealthcheck::PHP_NEXT_MIN_VERSION_CONFIG) . ' will soon be not supported by cipherguard, so consider upgrading your operating system or PHP environment.'; // phpcs:ignore
    ...
    {
        $servicesToMock = [
            CipherguardConfigWritableWebInstallerHealthcheck::class,
        ...
        $this->assertResponseContains('<div class="message error">The cipherguard config is not writable.</div>');
        $this->assertCannotStartSetup();
    }

    public function testWebInstallerSystemCheck_Gpg_Directory_Undefined()
    {
        Configure::write('cipherguard.gpg.backend', 'foo');
    ...
    private function assertCanStartSetup(): void
    {
        ...
        $this->assertResponseContains('Nice one! Your environment is ready for cipherguard.');
    ...
    {
        $this->assertResponseOk();
        $this->assertResponseContains('Oops!! Cipherguard cannot run yet on your server.');
    }
