...
trait UserAccessControlTrait
{
    ...
    {
        return new UserAccessControl($role, UuidFactory::uuid('user.id.' . $user), $user . '@cipherguard.com');
    }

    public function mockAdminAccessControl()
    {
        return new UserAccessControl(Role::ADMIN, UuidFactory::uuid('user.id.admin'), 'admin@cipherguard.com');
    }
